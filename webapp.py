...
def order():
    ...
    if request.method == 'POST':  # Post Forms. 
        ...
        if 'customer_id' in request.form: # looks for customer_id in the form, then proceeds. 
            ...
            execute_query(db_connection, query, customer_id).fetchall()
            return redirect(url_for('results_order'))
...
@app.route("/tickets/search", methods=['POST'])
def tickets_search():
    name = request.form['name']
    formatted_name = "%" + name + "%"
    query = "SELECT t.id, t.issue, t.creation_date, t.completed, d.name as developer, p.name as project \
        FROM Tickets t LEFT JOIN Developers d ON t.developer_id = d.id \
            LEFT JOIN Projects p ON t.project_id = p.id \
            WHERE d.name LIKE %s"
    values = (formatted_name,)

    cursor = db.execute_query(db_connection_arg=db_connection, query_arg=query, query_params=values)
    results = cursor.fetchall()
    cursor.close()
    return render_template("tickets.html", tickets_cursor = results)